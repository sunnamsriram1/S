...
read -s input
echo
key=$(python3 -c "import base64; print(base64.urlsafe_b64encode('{password}'.ljust(32, '0')[:32].encode()).decode())")
tmp_file=$(mktemp)
if ! python3 -c "from cryptography.fernet import Fernet; print(Fernet(key.encode()).decrypt(b'{encrypted}').decode())" > $tmp_file 2>/dev/null; then
