...
def main():
    try:
        ...
        if os.path.isfile(url_file):
            ...
            printc(YELLOW, "ℹ️  Enter fallback input (single URL or .txt file path).")
            user_input = input(f"{BLUE}🌐 Enter URL or .txt file path with ?id=: {RESET}").strip()
        ...
        if not url_list:
            printc(RED, "[!] No valid URLs with 'id=' parameter found. Exiting.")
            return

        extra_options = "--batch --dbs"
