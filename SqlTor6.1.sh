...
echo -e "${GREEN}🛡️  SQLMap via Tor - Smart Auto Runner [SqlTor6.sh]${RESET}"
...
while IFS= read -r url; do
    ...
    grep -E "parameter|sql injection|back-end DBMS|is vulnerable" "$log_file" | uniq | sed 's/^/   - /'
...
echo -e "⚠️ Vulnerable: ${vuln}"
echo -e "❌ Skipped or Failed: ${fail}"
